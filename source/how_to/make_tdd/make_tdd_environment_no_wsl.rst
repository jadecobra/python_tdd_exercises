...
* I add the Python_ code below in ``tests/magic.py`` in the :ref:`editor<2 editors>`
  ...
  - ``def test_failure``
    ...
    * ``test_failure`` is the name I used :ref:`method<functions>` for :ref:`this first test<test_failure>`
      ...
      .. IMPORTANT:: I can use any name for the :ref:`test method<functions>`, it MUST start with ``test_`` or unittest_ does NOT run the tests in it
...
* these are the steps I took to make a Python_ :ref:`Test Driven Development environment<what is a Test Driven Development Environment?>`
  ...
  - :ref:`make a Python file to hold the source code in the 'src' folder<how to make an empty file on Windows without WSL>`
  ...
  - :ref:`make a requirements file for the needed Python packages<how to write text to a file on Windows without WSL>`
  ...
  - :ref:`run the tests automatically<how to run the tests automatically in a virtual environment on Windows without WSL>`
...
You have seen me make a :ref:`Test Driven Development environment<what is a Test Driven Development Environment?>` for a project called ``magic`` on any Windows_ computer that does NOT have `Windows Subsystem for Linux`_. Would you like to :ref:`test AssertionError next?<AssertionError>`
