...
* I add a :ref:`function<what is a function?>` for the calls to the `random.choice method`_ with a :ref:`starred expression<starred expressions>` like I did in :ref:`test_w_unknown_arguments`, to use to remove repetition of ``random.choice`` from the test in ``test_person.py``
...
The difference between the call to the :ref:`factory function<test_factory_w_keyword_arguments>` and the expected :ref:`dictionary<what is a dictionary?>` in the test is, one has a year of birth and the other does a calculation with the year of birth. The other things are the same.
...
* I add a default value for ``last_name`` in the :ref:`factory function<test_factory_w_keyword_arguments>` in ``person.py`` to make it a choice
...
* I add a default value for ``sex`` in the :ref:`factory function<test_factory_w_keyword_arguments>` in ``person.py``
...
* I add a random person that covers all the possible cases because the four people I made in :ref:`test_factory_person_says_hello` now all use random values, and the :ref:`assertions<what is an assertion?>` are the same for each case because they use the ``first_name``, ``last_name`` and ``age`` :ref:`variables<what is a variable?>` that are sent in the call to ``src.person.factory``
...
* I remove the commented lines and the other people from :ref:`test_factory_person_says_hello` because they are now all the same
...
* I use the ``get_random_year_of_birth`` :ref:`function<what is a function?>` to replace the repetition of making the values for the ``this_year`` and ``year_of_birth`` in :ref:`test_factory_w_keyword_arguments`
...
* I use the ``get_random_year_of_birth`` :ref:`function<what is a function?>` to replace the repetition of making the values for the ``this_year`` and ``year_of_birth`` in :ref:`test_factory_w_optional_arguments`
...
* I use the ``get_random_year_of_birth`` :ref:`function<what is a function?>` to replace the repetition of making the values for the ``this_year`` and ``year_of_birth`` in :ref:`test_factory_person_says_hello`
...
I made a person named ``john`` in :ref:`test_factory_person_says_hello` with a year of birth of ``1580``, which means he is too old to be alive. That age is also outside the range I used for the random age, the oldest person the test makes is 120.

Maybe I made a mistake when typing his age and typed ``5`` instead of ``9``. How would I change the year of birth of a person made with the :ref:`factory function<test_factory_w_keyword_arguments>` if I cannot change the original year of birth?

* I could try updating the ``year_of_birth``
* I could try making a :ref:`function<what is a function?>` that takes a person and a new year of birth as inputs, and returns the person with the corrected age
* I could make a new :ref:`factory function<test_factory_w_keyword_arguments>` that returns a :ref:`dictionary<what is a dictionary?>` with ``year_of_birth`` as a :ref:`key<test_keys_of_a_dictionary>` which allows me to change it, then make another :ref:`function<what is a function?>` that calculates the age from the returned :ref:`dictionary<what is a dictionary?>` - this sounds like a lot of work, I would also have to rewrite the tests. No, thank you.
...
* I try to use the ``year_of_birth`` :ref:`key<test_keys_of_a_dictionary>`
  ...
  there is no ``year_of_birth`` :ref:`key<test_keys_of_a_dictionary>` in the :ref:`dictionary<what is a dictionary?>` returned by the :ref:`factory function<test_factory_w_keyword_arguments>`
...
* I add an :ref:`assertion<what is an assertion?>` for the age of ``john smith`` again
  ...
  the :ref:`factory function<test_factory_w_keyword_arguments>` uses the value for ``year_of_birth`` to calculate the age when it makes the :ref:`dictionary<what is a dictionary?>`.
...
I cannot update the ``year_of_birth`` :ref:`key<test_keys_of_a_dictionary>` because the :ref:`function<what is a function?>` returns a :ref:`dictionary<what is a dictionary?>` that does not have a ``year_of_birth`` :ref:`key<test_keys_of_a_dictionary>`.
...
* I remove the commented lines from the :ref:`factory function<test_factory_w_keyword_arguments>`
