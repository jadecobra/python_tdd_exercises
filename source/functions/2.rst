...
* ``w_keyword_arguments`` and ``positional_arguments`` are the same :ref:`functions<what is a function?>`, they always
...
* I add a :ref:`function definition<how to make a function>`
  ...
  - The :ref:`function definition (signature)<how to make a function>` of ``args_and_kwargs`` does not allow any inputs when it is called since the parentheses are empty.
  - I am violating the :ref:`function signature<how to make a function>` when I call it in a way that it was not designed to be called, which raises :ref:`TypeError<what causes TypeError?>`.
...
* I add the name in parentheses
  ...
  because the :ref:`definition<how to make a function>` for ``args_and_kwargs`` takes one argument (``last_input``), and the :ref:`assertion<what is an assertion?>` calls the :ref:`function<what is a function?>` with two arguments ``('first', last_input='last')``. How does Python_ know which value to use for ``last_input`` if I use the :ref:`position<test_positional_arguments>` and the :ref:`keyword<test_keyword_arguments>`?
...
I add the :ref:`function definition<how to make a function>` for ``optional_arguments``
...
* I remove ``last_input=last_name`` from the call to ``optional_arguments`` to show that it is a required argument
  ...
  .. code-block:: python
    ...
    TypeError: optional_arguments() missing 1
...
* I make the argument optional by giving it a default value
  ...
  the test passes since I do not need to give a value for the ``last_input`` parameter in the call to ``src.functions.optional_arguments`` because the :ref:`default value<test_optional_arguments>` for the ``last_input`` parameter of the :ref:`function<what is a function?>` is ``doe``. This means that

  .. code-block:: python

    src.functions.optional_arguments('jane')

  is the same as

  .. code-block:: python

    src.functions.optional_arguments('jane', last_input='doe')
  ...
  because ``optional_arguments`` will always
...
* I add another :ref:`assertion<what is an assertion?>` to show that I can still call the :ref:`function<what is a function?>` with different values, in :ref:`test_optional_arguments` in ``test_functions.py``
...
* I change ``my_expectation`` to match ``reality``
  ...
  the test passes since I do not need to give a value for the ``last_input`` parameter in the call to ``src.functions.optional_arguments`` because the :ref:`default value<test_optional_arguments>` for the ``last_input`` parameter of the ``optional_arguments`` :ref:`function<what is a function?>` is ``doe``. This means that

  .. code-block:: python

    src.functions.optional_arguments(first_input='john')
