...
* I will add a failing test to ``test_truth_table.py``
  ...
  and the terminal shows an :ref:`AttributeError` which I will add to the list of exceptions encountered
...
* I will add a :doc:`singleton function </functions/functions_singleton>` called ``logical_true`` to ``truth_table.py``
...
* I will add a test for ``logical_false`` to ``TestNullaryOperations`` class in ``test_truth_table.py``
...
* I will add a function definition for ``logical_false`` to ``truth_table.py``
...
* When I will make the return value to :doc:`False </data_structures/booleans/booleans>`, the terminal shows passing tests
...
I will add a new ``TestCase`` to ``test_truth_table.py``
...
I will add a function definition for ``logical_identity`` to ``truth_table.py``
...
I will add a test for ``logical_negation`` to ``test_truth_table.py``
...
* I will add a definition for ``logical_negation``  to ``truth_table.py`` using the solution I had for ``logical_identity``
