...
* I pick ``none`` as the name of this project
* I open a terminal_
...
* I `change directory`_ to the project
  ...
  the terminal_ shows I am now in the ``none`` folder_
...
* I use touch_ to make an empty file_ for the program_ in the ``src`` folder_
  ...
  .. admonition:: on Windows without `Windows Subsystem for Linux`_ use ``New-Item src/none.py`` instead of ``touch src/none.py``
...
* I make an empty file_ for the actual test
  ...
  .. admonition:: on Windows without `Windows Subsystem for Linux`_ use ``New-Item tests/test_none.py`` instead of ``touch tests/test_none.py``
...
* I hold ``ctrl`` (Windows_/Linux_) or ``option or command`` (MacOS_) on the keyboard and use the mouse to click on ``tests/test_none.py:7`` to open it in the :ref:`editor<2 editors>`
* then I change :ref:`True<test_what_is_true>` to :ref:`False<test_what_is_false>` in ``test_none.py``
