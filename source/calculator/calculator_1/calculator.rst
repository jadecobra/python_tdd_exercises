...
* I add :ref:`AssertionError<what causes AssertionError?>` to the list of :ref:`Exceptions<how to test if an Exception is raised>` seen, in ``test_calculator.py``
...
* I change :ref:`test_failure` to ``test_addition`` then change `assertFalse`_ to `assertEqual`_
  ...
  - the :ref:`assertEqual method<test_assert_equal>` from :ref:`AssertionError<what causes AssertionError?>` checks if the two things in parentheses are the same. It is like the statement ``assert x == y`` or asking ``is x equal to y?``
...
* I add :ref:`NameError<test_catching_name_error>` to the list of :ref:`Exceptions<how to test if an Exception is raised>` seen, in ``test_calculator.py``
...
* I add :ref:`AttributeError<what causes AttributeError?>` to the list of :ref:`Exceptions<how to test if an Exception is raised>` seen, in ``test_calculator.py``
...
* I add :ref:`TypeError<what causes TypeError?>` to the list of :ref:`Exceptions<how to test if an Exception is raised>` seen, in ``test_calculator.py``
...
* I can use any range of numbers the computer can handle, for example
  ...
  the terminal_ is my friend, and shows OverflowError_
...
* I delete all the text in ``calculator.py``, the terminal_ is my friend, and shows 4 failures
...
What other :ref:`Exceptions<how to test if an Exception is raised>` do you think are raised as I go along?
...
* I add :ref:`positional arguments<test_positional_arguments>` to the :ref:`function<what is a function?>`
  ...
  the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` with random numbers
...
* I change the :ref:`return statement<the return statement>` to see the difference between the inputs and expected output, remember the :ref:`identity function?<test_identity_function>`
  ...
  the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` with random numbers that look like this
...
* I add a :ref:`function<what is a function?>`
  ...
  I add 2 names for the :ref:`positional arguments<test_positional_arguments>`
...
:ref:`Would you like to see a better way to write test_why_use_a_function?<functions 2: use variables>`
