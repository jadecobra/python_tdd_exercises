...
.. literalinclude:: ../code/calculator/tests/test_calculator_4.py
  :language: python
  :linenos:
...
I want the :ref:`add function<test_addition>` to raise TypeError_ when it gets a string_, the same way the other :ref:`functions<what is a function?>` raise TypeError_ when one of the inputs is a string_. I can use the `isinstance function`_ which is like the :ref:`assertIsInstance method<another way to test if something is an instance>` from when I tested :ref:`None<what is None?>`, it checks if one thing is an instance or child of a :ref:`class<what is a class?>`
...
* then I add an :ref:`if statement<if statements>` to the :ref:`add function<test_addition>` in ``calculator.py``
  ...
  .. note::
    ...
    - the `isinstance function`_ like the :ref:`assertIsInstance method<another way to test if something is an instance>` checks if the first input it is given is an instance (child) of the :ref:`class<what is a class?>` it is given as the second input. It is part of `Python's Built-in Functions`_
...
Even though the program_ says it only works with numbers, I did not add tests for tuples_, :ref:`lists`, sets_, and :ref:`dictionaries<what is a dictionary?>`, though they are touched in :ref:`test_type_error_w_objects_that_do_not_mix`, Do you want to add them or do we already have enough tests to know what would happen?
