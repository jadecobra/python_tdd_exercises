...
* I will add a failing test to check that when the ``duration`` :ref:`function<functions>` in the ``sleep_duration`` :doc:`module </exceptions/ModuleNotFoundError>` is called with a ``wake_time`` of ``'08:00'`` and a ``sleep_time`` of ``'07:00'``, it should return the difference between the two, which in this case is ``1``
...
I will add a failing test in ``test_sleep_duration.py`` that has hours and minutes
...
* I will make the error message in ``test_duration_w_date_and_time`` to use `datetime.datetime`_ objects
