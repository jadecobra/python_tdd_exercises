...
* I make the return value to make it match the expectation
...
* I want to try one of the :ref:`methods<functions>` listed in ``test_string_attributes_and_methods`` to see if it will get me closer to a solution but I cannot tell what they do from the names. I use the `help system`_ to check the `python documentation for strings`_ for more details
...
* I want to split the string_ on a separator to get the different parts, something like ``['01', '23']`` with ``:`` as the separator. I make the expectation of the test to what I want
...
So far, the ``duration`` :ref:`function<functions>` has been tested with timestamps that only have hours and minutes, but I could fall asleep on a Monday and wake up on a Tuesday. What would happen if I added dates to the timestamps?
...
* I make the error message in ``test_duration_w_date_and_time`` to use `datetime.datetime`_ objects
