...
* then define ``duration`` as a :doc:`function </functions/functions>` in ``sleep_duration.py`` to make it callable_
  ...
  The ``duration`` :doc:`function </functions/functions>` is called in ``test_duration_w_hours`` with keyword arguments that have not been provided in the function signature when it is defined
