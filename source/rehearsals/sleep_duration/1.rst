...
* I want to try one of the :ref:`methods<functions>` listed in ``test_string_attributes_and_methods`` to see if it will get me closer to a solution but the names in the list do not give me enough information since I do not know what they do, so I use the `help system`_ to check the `python documentation for strings`_ for extra details
...
The ``duration`` :ref:`function<functions>` has been tested with timestamps that have only hours and minutes, but I could fall asleep on a Monday and wake up on a Tuesday. What would happen if I added dates to the timestamps?
