...
* I change the ``duration`` :ref:`function<functions>` in ``sleep_duration.py`` to use the `str.split`_ :ref:`method<functions>`
...
* I create a :ref:`function<functions>` to replace the ``get_hour`` and ``get_minutes`` functions
