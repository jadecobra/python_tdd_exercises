...
* I use the dir_ :ref:`function<functions>` to see what :ref:`methods<functions>` and :doc:`attributes </exceptions/AttributeError>` of strings_ can help me break a string_ apart or get the characters I want from it
...
* I will add an opening quote, there is a closing quote with no open quote
...
* I will add the suggestion
  ...
  - the terminal shows a list of :ref:`methods<functions>` and :doc:`attributes </exceptions/AttributeError>` of a string_
...
* I want to `str.split`_ the string_ on a ``separator`` so I get the separate parts, something like ``['01', '23']`` with ``:`` as the separator. I will make the expectation of the test to match this idea
...
* I will make the ``duration`` :ref:`function<functions>` in ``sleep_duration.py`` to use the `str.split`_ :ref:`method<functions>`
...
* I will make the test to match the expectation and we are green again
...
* I will add the conversion using the int_ constructor to the ``duration`` :ref:`function<functions>` to see if it makes the test pass
...
The ``duration`` :ref:`function<functions>` has been tested with timestamps that contain only hours and minutes, but I could fall asleep on a Monday and wake up on a Tuesday. What would happen if I will added dates to the timestamps?

red: make it fail
--------------------------------------------

* I will add a failing test to ``test_sleep_duration.py`` based on ``test_duration_w_hours_and_minutes`` and call it ``test_duration_given_date_and_time`` to test the ``duration`` function with a date, hours and minutes
...
* I will add tests using the examples in the documentation to help me understand how to use the datetime_ module
...
* I will add an `import statement`_ for the datetime_ module to ``test_sleep_duration.py``
...
* So far the `datetime.timedelta`_ object I get shows seconds, but I want the result as a string. I will add a test to see if I can change it to a string_ using the str_ constructor
...
* I will add a function for converting timestamps to ``sleep_duration.py`` and call it ``get_datetime_object``
...
* I will make ``test_duration_given_date_and_time`` to use the right error message
