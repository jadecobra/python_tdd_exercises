...
I want to make a **ParkingGarage** that heats up food or stays off, if the inputs are
...
* I name this project ``parking garage``
* I open a terminal_
...
.. code-block:: python
  ...
  class TestParkingGarage(unittest.TestCase):
      ...
      def test_open_door(self):
          ...
          reality = src.parking garage.parking garage(
              door_is_open=True,
...
* I add an `import statement`_ at the top of the file_
  ...
  .. code-block:: python
    ...
    AttributeError: module 'src.parking garage' has no attribute 'parking garage'

  because ``parking garage.py`` in the ``src`` folder_ does not have anything named ``parking garage`` in it
...
* I open ``parking garage.py`` from the ``src`` folder_
...
* I add a :ref:`function<what is a function?>` named ``parking garage`` to ``parking garage.py``
  ...
  because the test :ref:`called<how to call a function with input>` the ``parking garage`` :ref:`function<what is a function?>` with two keyword arguments and this definition only takes calls with 0 arguments
...
* I add the :ref:`keyword argument<test_keyword_arguments>` to the :ref:`function<what is a function?>` in ``parking garage.py``

  .. code-block:: python
    :linenos:
    :emphasize-lines: 1

    def parking garage(door_is_open):
        return None
  ...
  because the test :ref:`called<how to call a function with input>` the ``parking garage`` :ref:`function<what is a function?>` with two keyword arguments and this definition only takes calls with 1 input
...
* I change the :ref:`return statement<the return statement>` to give the test what it wants
  ...
  the test passes. The ``parking garage`` :ref:`function<what is a function?>` always returns :red:`OFF`, it does not care about the inputs. Is this :ref:`Contradiction?<test_contradiction>`
...
I add an :ref:`if statement<if statements>` to ``parking garage.py``
...
So far, the :ref:`truth table` for the ParkingGarage is
...
I want the parking garage to only heat up food when the timer is set, the inputs for the parking garage will then be
...
because the test :ref:`called<how to call a function with input>` the ``parking garage`` :ref:`function<what is a function?>` with 3 keyword arguments (``door_is_open``, ``timer_is_set`` and ``start_is_pushed``) and the :ref:`function<what is a function?>` only takes calls with 2 arguments (``door_is_open`` and ``start_is_pushed``)
...
* I add ``timer_is_set`` to the :ref:`function signature<what is a function?>` in ``parking garage.py``
  ...
  .. code-block:: python
    ...
    FAILED ...test_closed_door - TypeError: parking garage() missing 1 required positional argument:...
    FAILED ...test_open_door - TypeError: parking garage() missing 1 required positional argument:...
...
* I add a :ref:`default value<test_optional_arguments>` to make ``timer_is_set`` a choice
  ...
  .. code-block:: python
    ...
    src.parking garage.parking garage(
        door_is_open=True,
        start_is_pushed=False,
    )
...
I add a :ref:`if statement<if statements>` to the ``parking garage`` :ref:`function<what is a function?>` in ``parking garage.py``
...
* I remove the :ref:`if statement<if statements>` for when the timer is :red:`NOT set` because I do not need it anymore
  ...
  still green. This is what happens when the ``parking garage`` :ref:`function<what is a function?>` is called
...
the :ref:`truth table` for the ParkingGarage is
...
I want to add a failsafe to stop the ParkingGarage if it gets too hot. The inputs will then be

* is the door open?
* is the timer set?
* was the start button pushed?
* is the parking garage too hot?
...
because the test :ref:`called<how to call a function with input>` the ``parking garage`` :ref:`function<what is a function?>` with 4 keyword arguments (``door_is_open``, ``timer_is_set``, ``start_is_pushed`` and ``too_hot``) and the definition only takes calls with 2 required arguments (``door_is_open`` and ``start_is_pushed``) and 1 optional argument (``timer_is_set``)
...
* I add ``too_hot`` to the ``parking garage`` :ref:`function signature<what is a function?>` in ``parking garage.py``
...
* I add an :ref:`assertion<what is an assertion?>` for when the door is :green:`open`, the timer is :green:`set`, the start button is :green:`pushed` and the parking garage temperature is :red:`NOT too hot`
...
* I add a value for the ``too_hot`` parameter to the first :ref:`assertion<what is an assertion?>` in :ref:`test_open_door_timer_not_set` for when the door is :green:`open`, the timer is :red:`NOT set`, the start button is :green:`pushed` and the parking garage temperature is :green:`too hot`
...
* I add a value for the ``too_hot`` and ``timer_is_set`` parameters in the second :ref:`assertion<what is an assertion?>`, for when the door is :red:`closed`, the timer is :green:`set`, the start button is :green:`pushed`, and the parking garage temperature is :green:`too hot`
...
* I add a value for the ``too_hot`` parameter to the first :ref:`assertion<what is an assertion?>` in :ref:`test_too_hot_closed_door_timer_not_set`, for when the door is :red:`closed`, the timer is :red:`NOT set`, the start button is :green:`pushed`, and the parking garage temperature is :green:`too hot`
...
I ran tests for a ParkingGarage with these inputs:
...
the only time this ParkingGarage heats food is when the door is :green:`closed`, the timer is :green:`set`, the start button is :green:`pushed` and the parking garage temperature is :red:`NOT too hot`.
