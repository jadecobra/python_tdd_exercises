...
This is V of a program_ that calculates the difference between a given wake and sleep time.
...
* then delete all the text in ``sleep_duration.py`` the terminal_ is my friend, and shows :ref:`AttributeError<what causes AttributeError?>`
...
* I add a list of :ref:`Exceptions<errors>` seen in ``test_sleep_duration.py``
...
* and the missing name to ``sleep_duration.py``
  ...
  I add it to the list of :ref:`Exceptions<errors>` seen as well
...
* and point ``get_datetime`` to :ref:`None<what is None?>`
  ...
  another error for the list of :ref:`Exceptions<errors>` seen in ``test_sleep_duration.py``
...
* then change it to a :ref:`function<what is a function?>` to make it callable_
  ...
  the terminal_ is my friend, and shows this :ref:`AssertionError<what causes AssertionError?>`
...
* When I get the error with the message about ``wake_time`` being earlier than ``sleep_time``, I copy it from the terminal_ to change the message of the ValueError_
  ...
  the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` because the timestamps in the ValueError_ message are not the same
