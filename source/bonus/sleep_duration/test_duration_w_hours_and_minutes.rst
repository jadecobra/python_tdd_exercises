...
I want to test the ``duration`` :ref:`function<what is a function?>` with timestamps where both hours and minutes are random.
...
* and change the ``random_timestamp`` :ref:`function<what is a function?>` to have random numbers from ``0`` up to and including ``59`` for the minutes
...
* I make a copy of ``difference_hours`` in ``duration``, change the name, then add it to the `return statement`_
  ...
  the :ref:`function<what is a function?>` returns the same numbers for hours and minutes because ``difference_hours`` and ``difference_minutes`` are the same. I make a copy of the ``get_hour`` :ref:`function<what is a function?>`, call it ``get_minutes`` and change the index to get the second item from the timestamp split
...
The ``duration`` :ref:`function<what is a function?>` returns a subtraction of hours and a subtraction of minutes which does not give the right difference between the timestamps
...
the terminal_ is your friend, and shows :ref:`AssertionError<what causes AssertionError?>` when I add ``test_duration_calculation``
...
* I add a `return statement`_ to the ``duration`` :ref:`function<what is a function?>` where I multiply ``difference_hours`` by ``60`` then add it to ``difference_minutes`` to get the total difference in minutes
  ...
  the terminal_ is your friend, and shows :ref:`AssertionError<what causes AssertionError?>` for ``test_duration_w_hours_and_minutes``
  ...
  the terminal_ is your friend, and shows :ref:`AssertionError<what causes AssertionError?>` for ``test_duration_calculation``

  .. code-block:: shell

    AssertionError: 31 != '00:31'

  the :ref:`function<what is a function?>` returns the right number of minutes for the difference. I need a way to change it to hours and minutes to match the expectations of the tests. I add the `unittest.skip decorator`_ to skip it while I test the solution
...
* and comment it out for ``test_duration_w_hours_and_minutes``
  ...
  the terminal_ is your friend, and shows random successes and random :ref:`AssertionError<what causes AssertionError?>`
  ...
  the calculation in the test is still not right. I change it to match the ``duration`` :ref:`function<what is a function?>`
  ...
  then change the :ref:`variables<what is a variable?>` in the expectation
...
* then add a :ref:`function<what is a function?>` in ``sleep_duration.py`` to change ``get_hour`` and ``get_minutes``
...
* and remove ``get_hour`` and ``get_minutes``. the terminal_ is your friend, and shows all tests are still passing!
