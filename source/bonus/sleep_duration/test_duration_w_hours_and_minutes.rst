...
the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` when I add ``test_duration_calculation``
...
* I add a `return statement`_ to the ``duration`` :ref:`function<what is a function?>` where I multiply ``difference_hours`` by ``60`` then add it to ``difference_minutes`` to get the total difference in minutes
  ...
  the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` for ``test_duration_w_hours_and_minutes``
  ...
  the terminal_ is my friend, and shows :ref:`AssertionError<what causes AssertionError?>` for ``test_duration_calculation``
...
* and comment it out for ``test_duration_w_hours_and_minutes``
  ...
  the terminal_ is my friend, and shows random successes and random :ref:`AssertionError<what causes AssertionError?>`
...
* and remove ``get_hour`` and ``get_minutes``. the terminal_ is my friend, and shows all tests are still passing!
