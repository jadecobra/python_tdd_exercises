...
.. code-block::
   ...
   ```
   the terminal updates to show an [AttributeError](./ATTRIBUTE_ERROR.rst) and we add it to our list of exceptions encountered
...
* update the return statement to return an empty dictionary
  .. code-block:: python
       ...
       def a_dict():
           return {}
    the terminal still shows an `AssertionError <./ASSERTION_ERROR.rst>`_ but now our return value looks more similar to what is expected
  .. code-block:: python
...
the terminal updates to show an `AssertionError <./ASSERTION_ERROR.rst>`_ since the two values are different
...
* Our knowledge of dictionaries is growing. We know we can use ``integers`` and ``strings`` as dictionary keys. Can we use ``floats``? We are going to find out by adding a test
  .. code-block:: python
    ...
    the terminal updates to show an `AssertionError <./ASSERTION_ERROR.rst>`_ since the values are different
...
the terminal outputs an `AssertionError <./ASSERTION_ERROR.rst>`_
...
the terminal responds with a `TypeError <./ASSERTION_ERROR.rst>`_
...
and the terminal outputs a `TypeError <./TYPE_ERROR.rst>`_
