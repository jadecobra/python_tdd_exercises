
#################################################################################
AssertionError: tests
#################################################################################

----

*********************************************************************************
what is an assertion?: tests
*********************************************************************************

The code in ``assertion_error/tests/test_assertion_error.py`` from :ref:`what is an assertion?`

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :linenos:
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 1-6

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 9
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 9-28

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 31
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 31-50

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 53
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 53-72

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 75
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 75-86

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 89
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 89-103

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 106
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 106-118

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 119
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 119-127

.. literalinclude:: assertion_error/test_assertion_error.py
  :language: python
  :lineno-start: 128
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 128-

----

*********************************************************************************
test AssertionError with functions: tests
*********************************************************************************

The code in ``assertion_error/tests/test_assertion_error.py`` from :ref:`test AssertionError with functions`


.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :linenos:
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 1-10

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 13
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 13-22

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 25
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 25-28

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 31
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 31-42

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 45
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 45-56

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 59
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 59-70

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 73
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 73-80

.. literalinclude:: assertion_error/test_assertion_error_w_functions.py
  :language: python
  :lineno-start: 83
  :caption: assertion_error/tests/test_assertion_error.py
  :lines: 83-99

----

*********************************************************************************
test AssertionError with unittest: tests
*********************************************************************************

The code in ``assertion_error/tests/test_assertion_error.py`` from :ref:`test AssertionError with unittest`

.. literalinclude:: assertion_error/test_assertion_error_w_unittest.py
  :language: python
  :linenos:

----

*********************************************************************************
test AssertionError with assertIsNotNone and assertIsNone: tests
*********************************************************************************

The code in ``assertion_error/tests/test_assertion_error.py`` from :ref:`test AssertionError with assertIsNotNone and assertIsNone`

  .. literalinclude:: assertion_error/test_assertion_error_w_none.py
    :language: python
    :linenos:

----
