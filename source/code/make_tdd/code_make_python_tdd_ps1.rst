...
* Here is the ``./makePythonTdd.ps1`` program from :ref:`how to make a python test driven development environment on Windows without Windows Subsystem Linux`

  .. literalinclude:: makePythonTdd.ps1
    :language: PowerShell
    :linenos:

* give a name for the ``$PROJECT_NAME`` variable when the program is called to make a `Test Driven Development`_ any time you want. For example typing this command in the terminal_ in the folder where ``makePythonTdd.ps1`` is saved will make a `Test Driven Development`_ environment for a project called :ref:`calculator<how to make a calculator>`
